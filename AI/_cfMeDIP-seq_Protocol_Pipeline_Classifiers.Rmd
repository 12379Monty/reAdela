---
title:  "cfMeDIP-seq: Protocol, Pipeline, and Classifier Methodologies"
#subtitle: "a tale of two schools"
author: "claude.ai Opus 4.5 with naive prompts from [Francois Collin](https://www.linkedin.com/in/francoisz/)"
...
1. The initial protocol as described in Shen et al., Nature Protocols 2019 [@Shen2019]
2. Sources of technical variability in the protocol
3. The MEDIPIPE computational analysis pipeline
4. Machine l/earning classifiers developed for cancer detection
5. Protocol and classifier improvements over time

---

# Protocol: cfMeDIP-seq (Nature Protocols 2019) [@Shen2019]

## Overview

The cfMeDIP-seq protocol was published by Shen, Burgener, Bratman, and De Carvalho in Nature Protocols (2019)
[@Shen2019]  as an adaptation of conventional MeDIP-seq for low-input cell-free DNA samples. The protocol enables methylome profiling from 1-10 ng of input DNA through the addition of "filler DNA" before immunoprecipitation.
...
## **Sample Prep and Sequencing Protocol** {#prot-steps .tabset}

The cfMeDIP-seq protocol consists of four major stages:

* **STAGE 1: Library Preparation from cfDNA (Day 1)** - steps 1-3
* **STAGE 2: Methylated DNA Immunoprecipitation (Day 2)** - steps 4-10
   - <span style="color:red"> the core of the assay and where most of the variability comes from.  </span>
* **STAGE 3: Library Amplification (Day 3)** - steps 11-14
* **STAGE 4: High-Throughput Sequencing (Day 4 and beyond)** - step 15
...
```{r pipeline-flowchart, fig.width=8, fig.height=11}
###, fig.cap="cfMeDIP-seq Protocol (Shen et al. 2019)"}
...
- Or add synthetic spike-in controls (Wilson et al., 2022) [@Wilson2022]
- Time: ~30 minutes
...
The protocol includes several post-sequencing QC checkpoints:
...
*  Check these for correlation/association among themselves
...
#### Detailed Metrics by Stage {- .tabset}
...
## Improvements to Mitigate Variability {.tabset}
...
**Unique Molecular Identifiers (UMIs) (Burgener et al., 2021)** [@Burgener2021]
...
**Synthetic Spike-In Controls (Wilson et al., 2022 [@Wilson2022])**

(<span style="color:blue">TO VERIFY</span>)
...
- Differences in methylation patterns in the spiked fragments across sets are used
as a proxy for systematic differences in  methylation patterns in the non-spiked fragments.
   - this must be verified.
   - even when spike-ins provide a good proxy for normalization, estimates might be noisy

- What does absolute methylation look like - is it a global number or local quants?
     - Models can be fitted to site-specific methylation to provide estimates of the chance
of cancer.  What does absolute methylation levels add?
...
# Computational Pipeline: MEDIPIPE 

## Overview

MEDIPIPE is an automated, end-to-end pipeline for cfMeDIP-seq data quality control, methylation quantification, and sample aggregation, published by Zeng et al. in Bioinformatics (2023) [@Zeng2023].
The pipeline was developed using Snakemake for reproducibility and scalability.
...
## Shen et al. 2018 (Nature) [@Shen2018] - Foundational Classifier
...
## Halla-aho & Lähdesmäki 2022 (BMC Bioinformatics) [@Hallaaho2022] - Comprehensive Method Comparison
...
- Same as Shen et al. 2018 [@Shen2018]
...
## Nuzzo et al. 2019/2020 [@Nuzzo2019; @Nuzzo2020]  - Renal Cell Carcinoma Classifier
...
## De Pascali et al. 2024 [@DePascali2024]- BRCA1/2 Breast Cancer Classifier
...
**2018 - Shen et al. (Nature)** [@Shen2018]
...
**2019 - Shen et al. (Nature Protocols)** [@Shen2019]
...
**2021 - Burgener et al.** [@Burgener2021]
...
**2022 - Wilson et al. (Cell Reports Methods)** [@Wilson2022]
...
**2022 - Halla-aho & Lähdesmäki (BMC Bioinformatics)** [@Hallaaho2022]
...
**2023 - Zeng et al. (Bioinformatics)**  [@Zeng2023]
...
**2024 - De Pascali et al. (J Translational Med)** [@DePascali2024]
...
### Improvement 1: Unique Molecular Identifiers (UMIs)
...
(<span style="color:blue">TO VERIFY - Improvement 2 is
applied after  Improvement 1</span>)

### Improvement 2: Synthetic Spike-In Controls
...
**Solution (Wilson et al., 2022):** [@Wilson2022]
...
### Nassiri et al. 2020 [@Nassiri2020] - Intracranial Tumor Classifier
...
### Burgener et al. 2021 [@Burgener2021] - UMI Implementation for MRD
